...
public class Resources {
    public static String $i(String i) {
        URL inputUrl = Resources.class.getClassLoader().getResource(i);
        if (inputUrl == null) {
            URL rootSource = Resources.class.getClassLoader().getResource(".");
            throw new Error(String.format("Not found input file  '%s' in path %s ", i, rootSource.getPath()));
        }
    ...
    public static String outputDirectory() throws IOException {
        ...
        String rootDir = Enviroment.enviroment("ASPOSE_EXAMPLES_OUTPUT_DIR_RESULT", "aspsoe-examples-output");
    ...
    public static String $o(String o) {
        Path pathResource = null;
        try {
            pathResource = Paths.get(outputDirectory(), o);
